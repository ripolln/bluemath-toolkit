...
def fit_gev_kma_frechet(bmus, var):
    '''
    ...
    bmus  - KMA clusters time series  (numpy.array)
    var   - time series of variable to fit to GEV/Gumbel_L  (numpy.array)

    returns dictionary {cluster_id: gev_parameters}
            where gev_parameters = [shape, loc, scale]
    '''

    # get available clusters from bmus series
    list_clusters = np.unique(bmus)

    out_param_gev = {}
    for cluster in list_clusters:
        pos = np.where((bmus==cluster))[0]

        if len(pos) == 0:
            param_GEV = [np.nan, np.nan, np.nan]
        ...
        # store parameters at dict
        out_param_gev[cluster] = param_GEV

    return out_param_gev
...
def smooth_gev_shape(cenEOFs, param):
    ...
    cenEOFs  - KMA centroids  (numpy.array [n_clusters, n_features])
    param    - GEV shape parameter for each KMA cluster  (numpy.array)
