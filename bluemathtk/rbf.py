...
def cost_eps(ep, x, y):
    'calculate rbf cost function'

    # rbf coeff and A matrix calculation
    rbf_coeff, A = rbf_coefficient(ep, x, y)
...
def rbf_coefficient(ep, x, y):
    'rbf coefficient calculation'
...
def rbf_reconstruction(
    ...
    for ix in ix_scalar_target:
        ...
        opt_sigma = fminbound(
            cost_eps, sigma_min, sigma_max, args=(subset_norm.T, v)
        )
        t1 = time.time()  # optimization time

        # calculate RBF coeff
        rbf_coeff, _ = rbf_coefficient(opt_sigma, subset_norm.T, v)
    ...
    for ix in ix_directional_target:
        ...
        # calculate RBF coeff
        rbf_coeff_x, _ = rbf_coefficient(opt_sigma_x, subset_norm.T, vdx)
        rbf_coeff_y, _ = rbf_coefficient(opt_sigma_y, subset_norm.T, vdy)
